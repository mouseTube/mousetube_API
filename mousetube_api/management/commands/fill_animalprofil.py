...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        existing_keys = set(
            AnimalProfil.objects.values_list(
                "strain_id", "sex", "genotype", "treatment"
            )
        )
        ...
        def process():
            ...
            self.stdout.write(
                "🔄 Linking Subjects to their corresponding AnimalProfil..."
            )
            for subject in Subject.objects.all():
                ...
                if profil and subject.animal_profil != profil:
                    ...
                    self.stdout.write(
                        f"🔗 Linked Subject '{subject.name}' to AnimalProfil '{profil.name}'"
                    )
